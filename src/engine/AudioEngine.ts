...
export class AudioEngine {
  ...
  updateSettings(nextSettings: Partial<AudioEngineSettings>): void {
    ...
    if (this.micHighPass) {
      this.micHighPass.frequency.setTargetAtTime(this.settings.vocalIsolation ? 80 : 35, now, 0.02);
    }

    if (this.micLowPass) {
      this.micLowPass.frequency.setTargetAtTime(this.settings.vocalIsolation ? 1200 : 6000, now, 0.02);
    }
  ...
  updatePitchCorrection(frame: PitchFrame | null): void {
    if (!frame) {
      this.updateMonitorGate(0);
      if (this.pitchShift) {
  ...
  private updateMonitorGate(volume: number): void {
    const openThreshold = 10 ** (this.settings.noiseGateDb / 20);
    const closeThreshold = openThreshold * 0.55;
    const targetGate =
